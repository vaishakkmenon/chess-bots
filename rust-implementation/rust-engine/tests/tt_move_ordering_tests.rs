...
fn test_tt_stores_best_move() {
    ...
    // Search to depth 3
    let (score1, move1) = search_fixed_depth(&mut board, &tables, 3, &mut tt);
    ...
    let (score2, move2) = search_fixed_depth(&mut board, &tables, 4, &mut tt);
...
fn test_tt_move_ordering_improves_performance() {
    ...
    let _ = search_fixed_depth(&mut board1, &tables, 5, &mut tt_small);
    ...
    let start = Instant::now();
    let _ = search_fixed_depth(&mut board2, &tables, 5, &mut tt_large);
...
fn test_shallow_search_helps_deep_search() {
    ...
    let (_, shallow_move) = search_fixed_depth(&mut board, &tables, 2, &mut tt);

    println!("Shallow (depth 2) move: {:?}", shallow_move);

    // Now do a deep search (should use the TT move from shallow search)
    let start = Instant::now();
    let (_, deep_move) = search_fixed_depth(&mut board, &tables, 5, &mut tt);
...
fn test_tt_move_ordering_same_results() {
    ...
    // Search with TT
    let mut tt = TranspositionTable::new(64);
    let (score_with_tt, move_with_tt) = search_fixed_depth(&mut board1, &tables, 4, &mut tt);

    // Search again (fresh TT)
    let mut tt_fresh = TranspositionTable::new(64);
    let (score_fresh, move_fresh) = search_fixed_depth(&mut board2, &tables, 4, &mut tt_fresh);
...
fn test_tt_populated_during_search() {
    ...
    let (score1, _) = search_fixed_depth(&mut board, &tables, 4, &mut tt);

    // Second search of same position should be faster (TT hits)
    let start = Instant::now();
    let (score2, _) = search_fixed_depth(&mut board, &tables, 4, &mut tt);
    ...
    assert!(
        ...
        "Second search should be nearly instant with TT"
    );
}
...
fn test_tt_move_in_tactical_position() {
    ...
    let (score, best_move) = search_fixed_depth(&mut board, &tables, 3, &mut tt);
...
fn test_tt_move_is_always_legal() {
    ...
    let (_, best_move) = search_fixed_depth(&mut board, &tables, 4, &mut tt);
...
#[test]
#[ignore] // Run with: cargo test --release test_performance_comparison -- --ignored --nocapture
fn test_performance_comparison() {
    ...
    for depth in 4..=6 {
        ...
        let start = Instant::now();
        let (score, _) = search_fixed_depth(&mut board, &tables, depth, &mut tt);
